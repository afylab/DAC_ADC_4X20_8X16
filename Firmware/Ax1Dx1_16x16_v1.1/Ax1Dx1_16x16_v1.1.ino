...
//Updated: 07/01/2021 by Davis S. Thuillier -- added functions to send DAC code directly and set offset/gain error in Flash storage
//Updated: 08/17/2021 by Davis S. Thuillier -- modification to communicate with old DA16_16_02 DAC_ADC
...
int adc_sync[2] = {52, 42}; //CS for both 16-bit ADCs
int dacCS = 4; //CS for 4x 20-bit DAC channels
int ldac = 6; //LDAC pin shared across all AD5791 -- used to synchronize DAC voltage output
int reset[2] = {44, 46}; //reset pins on ADC
int drdy[2] = {48, 40}; //data_ready pin for both ADCs -- used as input to indicate ADC conversion has completed
...
float dacDataSend(int dacChannel, float voltage)
{
  ...
  SPI.transfer(16+(3-dacChannel)); // send command byte to DAC
  SPI.transfer(b1); // MS data bits, DAC2
  SPI.transfer(b2); //LS 8 data bits, DAC2
  digitalWrite(dacCS, HIGH);
  SPI.endTransaction();
  ...
  return GE[dacChannel]* (twoByteToVoltage(b1, b2) + OS[dacChannel]);
}
...
{
  dacDataSend(dacChannel, voltage / GE[dacChannel] - OS[dacChannel]);
}
...
float dacDataSend_buffer(int channelDAC, float voltage)
{
  ...
  SPI.beginTransaction(dacSettings);
  digitalWrite(dacCS, LOW);
  SPI.transfer(16+(3-channelDAC)); // send command byte to DAC
  SPI.transfer(b1); // MS data bits, DAC2
  SPI.transfer(b2); //LS 8 data bits, DAC2
  digitalWrite(dacCS, HIGH);
  SPI.endTransaction();
